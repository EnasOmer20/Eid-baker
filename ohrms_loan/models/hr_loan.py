...
class HrLoan(models.Model):
    ...
    state = fields.Selection(
        [('draft', 'Draft'), ('waiting_approval_1', 'Submitted'),
         ('approve', 'Approved'),('done', 'Done'), ('refuse', 'Refused'), ('cancel', 'Canceled'),
         ], string="State", default='draft', help="The current state of the "
                                                  "loan request.", copy=False)
    receipt_id = fields.Many2one('account.move', string="Receipt", readonly=True,
                                 help="Reference to the generated receipt.")
    ...
    def action_create_receipt(self):
        """ Method to create a receipt for the loan """
        self.ensure_one()
        if self.state != 'approve':
            raise UserError(_("You can only create a receipt for an approved loan."))

        partner_id = self.env['res.partner'].search([]).filtered(
            lambda p: self.employee_id.id in p.employee_ids.ids
        )

        # Ensure loan account and journal are set in the company
        if not self.env.company.loan_account_id:
            raise UserError(_("Please configure the Loan Account in the settings."))
        if not self.env.company.loan_journal_id:
            raise UserError(_("Please configure the Loan Journal in the settings."))
        if not partner_id:
            raise UserError(_("No contact found for the employee."))
        # Prepare receipt values
        move_vals = {
            'move_type': 'out_receipt',
            'partner_id': partner_id.id,  # Employee's contact
            'ref': f'Loan Receipt - {self.name}',
            'invoice_date': fields.Date.today(),
            'journal_id': self.env.company.loan_journal_id.id,  # Use the loan journal from settings
            'currency_id': self.currency_id.id,
            'invoice_line_ids': [(0, 0, {
                'name': f'Loan Amount for {self.employee_id.name}',
                'quantity': 1,
                'price_unit': self.loan_amount,
                'account_id': self.env.company.loan_account_id.id,  # Use the loan account from settings
            })],
        }

        # Create the receipt
        move = self.env['account.move'].create(move_vals)
        # move.action_post()  # Post the receipt

        # Link the receipt to the loan
        self.write({'receipt_id': move.id, 'state': 'done'})
    ...
    def action_view_receipt(self):
        """ Smart button to show receipt """
        self.ensure_one()
        if not self.receipt_id:
            raise UserError(_("No receipt created for this loan."))
        return {
            'type': 'ir.actions.act_window',
            'res_model': 'account.move',
